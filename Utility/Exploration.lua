...
function final()
    global:printSuccess("Script exploration terminé !" )
    explorationIsFinished = 1
end
...
starting = 1
fight = 0
function module:goOtomai(fightPar)
  fight = fightPar
    if starting == 1 then
      starting = 0
      return {
...
function buyHDV()
  ...
  global:delay(1000)
  if inventory:itemCount(2618) then
...
startingPandala = 1
function module:goPandala()
    if startingPandala == 1 then
      startingPandala = 0
      return {
    	   {map = map:currentMapId(), path="havenbag"}
       }
